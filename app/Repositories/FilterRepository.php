...
//    }
    ...
    public function filterProductCategoryWise($category_id, $category_ids, $sort_by, $paginate_by)
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ip-api.com/json/".$ipAddress."?lang=en");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($ch);
        $response = json_decode($response);
        curl_close($ch);

        $country = strtolower($response->country);
        $city = strtolower($response->city ?? '');
        ...
        $products = SellerProduct::query()
            ...
            ->where(function ($query) use ($category_id) {
                $query->where('categories.id', $category_id)
                    ->orWhereRaw('FIND_IN_SET(?, categories.parent_ids) > 0', [$category_id]);
            })
            ->where('products.country', 'LIKE', '%' . $country . '%') // Ülkeye göre filtreleme
            ->where(function ($query) use ($city) { // Şehire göre filtreleme
                if (!empty($city)) {
                    $query->where('products.country_level1', 'LIKE', '%' . $city . '%')
                        ->orWhereNull('products.country_level1');
                }
            })
