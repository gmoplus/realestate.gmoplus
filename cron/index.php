...
function performAction($action)
{
    ...
    while (count($listings) == $limit || $first) {
        ...
        foreach ($listings as $key => $listing) {
            ...
            $listing_title = $rlListings->getListingTitle($listing['Category_ID'], $listing, $listing_type['Key'], $listing['Lang']);
...
foreach (AsyncTasks::get() as $task) {
    AsyncTasks::execute($task['ID']);
}
