...
class rlFormats
{
    ...
    {
        global $_response, $lang, $insert, $config, $rlDb, $reefless;
    ...
    {
        global $_response, $reefless, $rlDb;
    ...
    {
        global $_response, $lang, $config, $rlValid, $reefless, $rlDb;
        ...
        if ($errors) {
            ...
            if ($default) {
                $parent_id = (int) $rlDb->getOne('ID', "`Key` = '{$format}'");
            ...
            if ($GLOBALS['rlActions']->updateOne($update, 'data_formats')) {
                /* update item name */
                foreach ($languages as $lang_item) {
                    if ($rlDb->getOne('ID', "`Key` = 'data_formats+name+{$item_key}' AND `Code` = '{$lang_item['Code']}'", 'lang_keys')) {
                        $lang_keys[] = array(
                            'fields' => array(
                                'Value' => $names[$lang_item['Code']],
                            ),
    ...
    {
        global $_response, $lang, $key, $reefless;
        ...
        /* delete item */
        $reefless->query("DELETE FROM `{db_prefix}data_formats` WHERE `Key` = '{$key}' LIMIT 1");

        /* delete phrases */
        $reefless->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'data_formats+name+{$key}'");
    ...
    {
        global $_response, $lang, $config, $id, $reefless, $rlDb;
        ...
        // get format ID
        $id = (int) $rlDb->getOne('ID', "`Key` = '{$key}'", 'data_formats');
        ...
        if (!$config['trash']) {
            // get child keys
            $child_keys = $rlDb->fetch(array('Key'), array('Parent_ID' => $id));

            // remove items lang keys
            foreach ($child_keys as $cKey => $cVal) {
                $rlDb->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'data_formats+name+{$cVal['Key']}'");
            }

            // remove child items
            $rlDb->query("DELETE FROM `{db_prefix}data_formats` WHERE `Parent_ID` = '{$id}'");
        }
