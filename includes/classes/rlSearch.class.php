...
class rlSearch
{
    ...
    {
        global $rlCache, $config, $rlListingTypes, $rlDb;
        ...
        if ($config['cache']) {
            ...
            if ($tab_form && $arrange_field) {
                $a_field = $rlDb->fetch(array('ID', 'Key', 'Type'), array('Key' => $arrange_field), null, 1, 'listing_fields', 'row');
                if ($a_field) {
    ...
    {
        global $sql, $custom_order, $rlListings, $rlCommon, $rlDb, $reefless;
        ...
        if ($data['myads_controller'] && $GLOBALS['account_info']['ID']) {
            $sql .= sprintf("WHERE `T1`.`Account_ID` = %d ", (int) $GLOBALS['account_info']['ID']);
        } else {
...
//        }
    ...
    public function keywordSearch($query = '', $mode = 3)
    {
        ...
        $mode = (int) ($mode ?: $GLOBALS['config']['keyword_search_type']);
        $query = trim($query);
        ...
        $multilingualFields = array_filter($fields, static function ($field) {
            return $field['Multilingual'] === '1';
        });

        $fieldKeys = [];
        foreach ($fields as $field) {
            $fieldKeys[$field['Key']] = $field['Key'];
        }
        ...
        switch ($mode) {
            ...
            case 2:
                ...
                foreach ($query_exploded as $query_item) {
                    ...
                    $sub_sql .= "(CONCAT_WS(' ', `T1`.`" . implode("`, `T1`.`", $fieldKeys) . "`, `TL`.`Value`) ";
                    $sub_sql .= "RLIKE '{$query_item}{$sign}') {$condition} ";
                }
                $sub_sql = preg_replace('/' . $condition . ' $/', '', $sub_sql);
                break;

            case 3;
                $startCondition = !$multilingualFields ? '(^|[[:space:]])' : '';
                $endCondition   = !$multilingualFields ? '([[:space:]]|$)' : '';
                $sub_sql = "CONCAT_WS(' ', ' ', `T1`.`" . implode("`, `T1`.`", $fieldKeys) . "`, `TL`.`Value`, ' ') ";
                $sub_sql .= "RLIKE '{$startCondition}{$query}{$endCondition}' ";
                break;
        }
    ...
    public function ajaxSaveSearch($type = '', $accountID = 0, $formKey = '')
    {
        ...
        foreach ($content as $key => $value) {
            ...
            if (is_array($content[$key]) && ($content[$key]['day'] && (int) $content[$key]['day'] < 0)
                && ($content[$key]['time'] && (int) $content[$key]['time'] < 0)
            ) {
                unset($content[$key]);
            }
