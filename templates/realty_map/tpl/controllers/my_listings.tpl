...
{if !empty($listings)}
    ...
    <script class="fl-js-dynamic">{literal}
        $(document).ready(function(){
            ...
            // Refresh sistemi için JavaScript kodu
            function checkRefreshAvailability() {
                console.log('Refresh durumu kontrol ediliyor...');
                $('.my-listings .item').each(function() {
                    var $listingItem = $(this);
                    var listingId = $listingItem.attr('id').replace('listing_', '');
                    ...
                    if (listingId) {
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {
                                action: 'checkRefreshAvailability',
                                listing_id: listingId
                            },
                            dataType: 'json',
                            success: function(response) {
                                console.log('Listing ' + listingId + ' refresh durumu:', response);
                                if (response.status == 'success') {
                                    var $refreshButton = $listingItem.find('.refresh-listing');
                                    var $refreshInfo = $listingItem.find('.refresh-info');
                                    
                                    if (response.canRefresh) {
                                        $refreshButton.removeClass('disabled').prop('disabled', false);
                                        $refreshInfo.text('Kalan yenileme: ' + response.remainingRefreshes);
                                    } else {
                                        $refreshButton.addClass('disabled').prop('disabled', true);
                                        $refreshInfo.text(response.message);
                                    }
                                }
                            },
                            error: function() {
                                console.log('Refresh durumu kontrol edilemedi: ' + listingId);
                            }
                        });
                    }
                });
            ...
            function refreshListing(listingId) {
                console.log('İlan yenileniyor:', listingId);
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function() {
                        $('#refresh_' + listingId).prop('disabled', true).text('Yenileniyor...');
                    },
                    success: function(response) {
                        console.log('Refresh yanıtı:', response);
                        if (response.status == 'success') {
                            printMessage('notice', response.message);
                            // Sayfa yenilenmeden durumu güncelle
                            setTimeout(function() {
                                checkRefreshAvailability();
                            }, 1000);
                        } else {
                            printMessage('error', response.message);
                            $('#refresh_' + listingId).prop('disabled', false).text('Yenile');
                        }
                    },
                    error: function() {
                        printMessage('error', 'Yenileme işlemi başarısız oldu.');
                        $('#refresh_' + listingId).prop('disabled', false).text('Yenile');
                    }
                });
            }

            // Refresh butonları için event handler
            $(document).on('click', '.refresh-listing', function(e) {
                e.preventDefault();
                var listingId = $(this).data('listing-id');
                if (listingId && !$(this).hasClass('disabled')) {
                    refreshListing(listingId);
                }
            });

            // Sayfa yüklendiğinde refresh durumunu kontrol et
            checkRefreshAvailability();
        });
        {/literal}
    </script>
