    ...
    <div class="main-column relative clearfix">
        <a {{if gc == 1}}target="_blank"{{/if}} href="{{if gc > 1}}javascript://{{else}}${url}{{/if}}">
            <div class="picture{{if !img}} no-picture{{/if}}">
                ...
                {{if fd == 1 && gc == 1}}<div class="label" title="{/literal}{$lang.featured}{literal}"><div>{/literal}{$lang.featured}{literal}</div></div>{{/if}}
                {{if gc > 1}}<mark class="group"><span>{{html String(count_in_location).replace(/(\[count\])/gi, gc)}}</span></mark>{{/if}}
            </div>
        </a>
        {{if price}}
        <div class="price-tag{{if srk == 2 && tf}} rent{{/if}}">
            <span>${price}</span>
            {{if srk == 2 && tf}}/ ${tf}{{/if}}
        </div>
        {{/if}}
        ...
        {{if gc == 1}}
            <ul class="nav-column{{if price}} stick-top{{/if}}">
