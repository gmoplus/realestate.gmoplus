...
// price history lib
var priceHistoryClass = function (data_range, value_range) {
    var self = this;

    this.char_drawed = false;
    this.box_width = "";
    this.table_height = "";
    this.chart_height = "";

    /**
     * Fire when user click by price-history tabs
     *
     * @since 1.2.2 - Removed "is_old" parameter
     */
    this.tabsHandler = function () {
        var table_height = $("#area_price_history_table").height();

        $('#tab_price_history_chart').click(function () {
            $("#area_price_history_chart").height(table_height);
            self.table_height = table_height;
            self.box_width = $("#area_price_history_table").width();
            if (!self.char_drawed) {
                self.drawChart();
            }
        });
    ...
    this.drawChart = function () {
        var formatedDate = []; // final data in X-ordinate
        for (i = 0; i < data_range.length; i++) {
            tmpDate = $.datepicker.formatDate('d M, yy', data_range[i]);
            tmpDateTime = data_range[i].getTime()
            formatedDate.push(tmpDate);
        }
        ...
        //set chart color by main sites colors.
        theme_main_colors = $("#line_color").css('color');
        chart.on('draw', function (data) {
            if (data.type === 'line') {
                data.element.attr({
                    style: "stroke:" + theme_main_colors + ";"
                });
            }
            if (data.type === 'point') {
                data.element.attr({
        ...
        $(document).on('mouseenter', '.ct-point', function () {
            var point_index = $(this).attr('point_index');
            var seriesName = formatedDate[point_index];
            var value = "";
            if (lang.system_currency_position === 'before') {
                value = self.shortPrice(lang.sys_currency + " " + $(this).attr('ct:value'));
            } else {
                value = self.shortPrice($(this).attr('ct:value') + " " + lang.sys_currency);
            }
        ...
        self.char_drawed = true;
    }

    /**
     * Build coordinates by X and Y ordinates
     *
     * @param min_date - Minimal date from range
     * @param max_date - Maximal date from range
     * @param days
     */
    this.BuildRange = function (min_date, max_date, days) {
        var dateArray = [], currentDate = min_date;
    ...
    /**
     * Check it value exist in array
     *
     * @value  - searching item
     * @array  - array
     * @return index - index of founded element
     */
...
/**
 * Add n-days to the Date object
 *
 * @days (int) - increase days
 * @return new date obj
 *
 **/
