    ...
    */
    ...
    public function hookApMixConfigItem(&$option)
    {
        ...
        switch ($option['Key']) {
            ...
            case 'locationFinder_mapping_address':
            case 'locationFinder_mapping_zip':
                $rlDb->setTable('listing_fields');
                $option['Values'] = array();

                if (!$fields) {
                    $fields = $rlDb->fetch(array('Key'), array('Status' => 'active'), "AND `Type` IN ('text','select') ORDER BY `Map` DESC, `Key`");
                }
