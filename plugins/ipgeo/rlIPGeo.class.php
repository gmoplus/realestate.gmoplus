...
class rlIPGeo extends Flynax\Abstracts\AbstractPlugin implements Flynax\Interfaces\PluginInterface
{
    ...
    public function hookInit()
    {
        ...
        // Return if the database is missing or uninstalled
        if (!$GLOBALS['config']['ipgeo_database_version'] || !file_exists($this->database)) {
            return;
        }
        ...
        if (false === $GLOBALS['reefless']->isBot() && null !== $ip = Util::getClientIP()) {
            ...
            // Global IP
                ...
                try {
                    ...
                    $location->Country_names = self::adaptLanguageCodes($record->country->names);
                    $location->Region        = $record->mostSpecificSubdivision->name;
                    $location->Region_names  = self::adaptLanguageCodes($record->mostSpecificSubdivision->names);
                    $location->City          = $record->city->name;
                    $location->City_names    = self::adaptLanguageCodes($record->city->names);
     ...
     * @since 2.0.1
     *
     * @param array|null $locations - List of countries/regions/cities/
     *
     * @return array
     */
    protected static function adaptLanguageCodes(?array $locations = []): array
    {
        ...
        foreach ($locations as $locationCode => $location) {
            if (false !== strpos($locationCode, '-')) {
                $locationCode = preg_replace('/-(.*)$/', '', $locationCode);
            }

            $adaptedLocations[$locationCode] = $location;
        }
     ...
     * @return void
     */
    public function printNotice(array &$notices): void
    {
        // Check version
        ...
        // Print system notice
        else if (version_compare($GLOBALS['config']['ipgeo_database_version'], '1.0.0') < 0) {
            $notices[] = preg_replace(
                '/(\[(.*)?])/', '<a href="' . RL_TPL_BASE . 'index.php?controller=ipgeo">$2</a>',
                $GLOBALS['rlLang']->getSystem('ipgeo_update_notice')
            );
        ...
        $GLOBALS['reefless']->createCookie('lf_last_version_check', 1, time() + 86400);
    }
