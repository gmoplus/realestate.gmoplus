...
{
    /**
     * @var mixed - Validator class object
     */
    public $rlValid;
    ...
    /**
     * @var mixed - Configurations class object
     */
    public $rlConfig;
    ...
    **/
    function ajax_addLanguage( $data )
    {
        ...
        else
        {
            ...
            $lang_exist = $this->rlDb->fetch('*', array( 'Code' => $iso_code ), null, null, 'iflynax_languages');
        ...
        else
        {
            ...
            $this->rlDb->setTable('iflynax_phrases');

            $copy_from_key = $this->rlDb->getOne('Key', "`Code` = '{$source_code}'", 'iflynax_languages');
            $source_phrases = $this->rlDb->fetch('*', array('Code' => $source_code), "AND `Key` <> '{$copy_from_key}'");
            ...
            if ( !empty($source_phrases) )
            {
                ...
                foreach ( $source_phrases as $item => $row )
                {
                    ...
                    if ($step % 500 == 0)
                    {
                        $this->rlDb->insert($insert_phrases, 'iflynax_phrases');
                ...
                $this->rlDb->insertOne($additional_row, 'iflynax_phrases');
            }
            else
            {
            ...
            else
            {
                ...
                $this->rlDb->insertOne($insert, 'iflynax_languages');
    ...
    **/
    function ajaxSetDefault( $object, $code )
    {
        ...
        if ($this->rlConfig->setConfig('iflynax_lang', $code)) {
            $_response -> script("
                languagesGrid.reload();
    ...
    **/
    function ajaxDeleteLang( $id )
    {
        ...
        $id = (int)$id;
        $code = $this->rlDb->getOne('Code', "`ID` = '{$id}'", 'iflynax_languages');

        if ( !$code || !$id )
            return $_response;

        if ( $config['iflynax_lang'] != $code )
        {
            $this->rlDb->query("DELETE FROM `".RL_DBPREFIX."iflynax_phrases` WHERE `Code` = '{$code}'");
            $this->rlDb->query("DELETE FROM `".RL_DBPREFIX."iflynax_languages` WHERE `Code` = '{$code}'");
    ...
    **/
    function ajax_addPhrase( $data, $values )
    {
        ...
        //check key exists
        $key_exist = $this->rlDb->fetch( 'ID', array( 'Key' => $key ), null, null, 'iflynax_phrases', 'row' );
        ...
        else
        {
            ...
            if ($this->rlDb->insert($phrase, 'iflynax_phrases')) {
    ...
    **/
    function exportLanguage( $id = false )
    {
        ...
        $info = $this->rlDb->fetch(array('Code', 'Key', 'Direction', 'Date_format'), array('ID' => $id), null, 1, 'iflynax_languages', 'row');
        $phrases = $this->rlDb->fetch(array('Value', 'Key'), array('Code' => $info['Code']), null, null, 'iflynax_phrases');

        if ( $phrases )
        {
            $content = <<<VS
<?xml version="1.0" encoding="UTF-8" ?>
<phrases direction="{$info['Direction']}" date_format="{$info['Date_format']}">\r\n
VS;
...
VS;
     ...
     */
    function ajaxCopyPhrases($from = false, $to = false)
    {
        global $_response, $lang;
        ...
        foreach ($phrases as $key => $value) {
            ...
            $this->rlDb->insertOne($insert, 'lang_keys');

            $compare_to[] = array(
                'ID'    => $this->rlDb->insertID(),
                'Code'  => $lang_code,
                'Key'   => $phrases[$key]['Key'],
                'Value' => $phrases[$key]['Value']
            );
