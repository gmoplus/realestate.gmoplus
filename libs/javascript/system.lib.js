...
/**
 * Cookie functions fallback (in case cookie.js loads after system.lib.js)
 */
...
var flynaxClass = function () {
  ...
  this.registration = function (type) {
    ...
    $('input[name="profile[password]"]').blur(function () {
      if ($('input[name="profile[password]"]').val().length < 3) {
        printMessage(
          "error",
          lang["notice_reg_length"].replace("{field}", lang["password"]),
          "profile[password]",
        );
      } else {
        check_repeat = true;
        ...
        if (
          rlConfig["account_password_strength"] &&
          $("#password_strength").val() < 3
        ) {
    ...
    $('input[name="profile[password_repeat]"]').blur(function () {
      /* clear field status */
      if (
        $(this).next().hasClass("fail_field") ||
        $(this).next().hasClass("success_field")
      ) {
        $(this).next().remove();
      }
      ...
      if (!$(this).val()) {
        $(this)
          .addClass("error")
          .after('<span class="fail_field">&nbsp;</span>');
        return;
      }
      ...
      if (pass != "" && check_repeat) {
        if ($(this).val() != pass) {
          printMessage(
            "error",
            lang["notice_pass_bad"],
            "profile[password_repeat]",
          );
          ...
          $(this)
            .removeClass("error")
            .after('<span class="success_field">&nbsp;</span>');
        }
      }
    });
  ...
  this.accountTypeChange = function (obj, direct) {
    ...
    var atype_key = $('[name="profile[type]"]')
      .find('option[value="' + atype_id + '"]')
      .data("key");
  ...
  this.passwordStrength = function () {
    ...
    var doMatch = function (obj) {
      ...
      $("div.password_strength div.scale div.color")
        ...
        .attr("class", "")
        .addClass("color")
        .addClass(scale[strength]);
      $("div.password_strength div.scale div.shine").width(strength * 20 + "%");
      $("#pass_strength").html(
        lang["password_strength_pattern"]
          .replace("{number}", strength)
          .replace("{maximum}", 5),
      );
  ...
  this.langSelector = function () {
    ...
    $(document).click(function (event) {
      ...
      if (
        $(event.target).parent().attr("class") == "bg" ||
        $(event.target).attr("class") == "bg" ||
        $(event.target).attr("class") == "arrow" ||
  ...
  this.paymentGateway = function () {
    ...
    var loadPaymentForm = function (gateway, type) {
      ...
      if (type == "default") {
        ...
        flUtil.ajax(
          { mode: "loadPaymentForm", gateway: gateway },
          function (response, status) {
            if (status === "success" && response.status === "OK") {
              response.html
                ? $customForm.html(response.html)
                : $customForm.html("");
            } else {
    ...
    $('ul#payment_gateways li input[type="radio"]').each(function () {
      if ($(this).is(":checked")) {
        loadPaymentForm(
          $(this).val(),
          $(this).parent().parent().data("form-type"),
        );
      }
    });
  ...
  this.isURL = function (str) {
    return /^(https?):\/\/((?:[a-z0-9.-]|%[0-9A-F]{2}){3,})(?::(\d+))?((?:\/(?:[a-z0-9-._~!$&'()*+,;=:@]|%[0-9A-F]{2})*)*)(?:\?((?:[a-z0-9-._~!$&'()*+,;=:\/?@]|%[0-9A-F]{2})*))?(?:#((?:[a-z0-9-._~!$&'()*+,;=:\/?@]|%[0-9A-F]{2})*))?$/i.test(
      str,
    );
  };
  ...
  this.treeLoadLevel = function (tpl, callback, section) {
    if (tpl == "crossed") {
      $("div.tree ul li > img:not(.no_child), div.tree li.locked a:not(.add)")
        .unbind("click")
        .click(function () {
      ...
      $("ul.select-category select")
        ...
        .bind("change", function () {
          ...
          if (
            typeof window["OnCategorySelect"] === "function" &&
            $(this).val()
          ) {
            var id = parseInt(
              $(this).find("option:selected").attr("id").split("_")[2],
            );
      ...
      $("ul.select-type li input")
        ...
        .click(function () {
          ...
          if (
            li.find("select > option").length == 2 &&
            li.find("select > option:eq(1)").hasClass("no_child")
          ) {
            $("a#next_step")
              .attr("href", li.find("select > option:last").val())
            ...
            if (li.find("select:first").val() == "") {
              ...
              $("a#next_step")
                .attr("href", li.find("select:last").val())
                .removeClass("disabled");
            }
          }
        });
      ...
      if (ct_parentPoints.length > 0) {
        var val = $(
          "ul.select-category option#tree_cat_" + ct_parentPoints[0],
        ).val();
        $(
          "ul.select-category li#type_section_" +
            ct_selectedType +
            " select:last",
        )
          .val(val)
        ...
        var val = $(
          "ul.select-category option#tree_cat_" + ct_selectedID,
        ).val();
        $(
          "ul.select-category li#type_section_" +
            ct_selectedType +
      ...
      $("ul.select-category span.tmp_info a")
        .unbind("click")
        .click(function () {
          $(this).parent().hide();
          $(this).parent().next().show();
        });
      $("ul.select-category span.tmp_input span.red")
        .unbind("click")
        .click(function () {
          $(this).parent().hide();
          $(this).parent().prev().show();
  ...
  this.treeLoadAction = function (obj, tpl, callback, section) {
    ...
    if ($(obj).find("option:selected").attr("id")) {
      var id = parseInt(
        $(obj).find("option:selected").attr("id").split("_")[2],
      );
      if (!section) {
        var set_section = $(obj).attr("class").replace("section_", "");
      } else {
        var set_section = $(obj).attr("class")
          ? $(obj).attr("class").replace("section_", "")
          : section;
      }
  ...
  this.slideTo = function (selector) {
    ...
    if (
      top_offset > pos.top ||
      pos.top > bottom_offset ||
      pos.top + $(selector).height() > bottom_offset
    ) {
      $("html, body")
        .stop()
        .animate({ scrollTop: pos.top - 10 }, "slow");
    }
  };
  ...
  this.highlightSRGrid = function (query) {
    ...
    $(
      "#listings div.list div.item td.fields>span,#listings table.table div.item td.fields td.value",
    ).each(function () {
  ...
  this.highlightSRDetails = function (query) {
    ...
    $("table.listing_details td.details table.table td.value").each(
      function () {
        var value = trim($(this).html());
        var href = false;
  ...
  this.planClick = function (plans) {
    ...
    $plan.click(function (e) {
      ...
      if (
        $(input).hasClass("hide") &&
        !$(this).find("input:not(.hide):checked").length
      ) {
        $(this)
          .find("input:not(.hide):not(:disabled)")
          .first()
          .prop("checked", true);
      }
    });
...
$(document).ready(function () {
  if (!readCookie("client_utc_time")) {
    var client_offset = new Date().getTimezoneOffset(),
      o = Math.abs(client_offset);
    var client_utc =
      (client_offset < 0 ? "plus" : "minus") +
...
(function ($) {
  $.fn.mCustomScrollbar = function (options) {
    ...
    $(this)
      .addClass(
        "scrollbar" +
          (settings.horizontalScroll ? " scrollbar__horizontal" : ""),
      )
      .css({ overflow: "auto" });
  };
})(jQuery);
